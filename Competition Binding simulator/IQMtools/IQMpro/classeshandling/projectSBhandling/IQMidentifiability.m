...
if nargout ~= 0,
    ...
    if pValueFlag == 0,
        ...
        axis square;
        colorbar('EastOutside','YTick',[-1,-0.8,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.8,1]);
        set(axesH2,'XTick',[1.5:size(plotData,1)+0.5]);
        set(axesH2,'XTickLabel',paramcorrdata.parameters);
        try
            set(gca,'XTickLabelRotation',90);
        catch
        end
        set(axesH2,'YTick',[1.5:size(plotData,1)+0.5]);
        set(axesH2,'YTickLabel',paramcorrdata.parameters);
        colormap('Bone');
        hlhlx = title(titleText);
        set(hlhlx,'Interpreter','none');
