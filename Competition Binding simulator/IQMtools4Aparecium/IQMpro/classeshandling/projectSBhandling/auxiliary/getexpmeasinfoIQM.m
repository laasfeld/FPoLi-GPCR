...
if isempty(compiledExpModelsIQMparamestGUI),
    ...
    if doPrecompile
        for i = 1 : numel(modelexperiments)
           compiledExpModelsIQMparamestGUI(modelindex).experiment(experimentindices(i)).modexp = modelexperiments{i};
           compiledExpModelsIQMparamestGUI(modelindex).experiment(experimentindices(i)).MEXmodel = mexmodelexperiments{i};
           compiledExpModelsIQMparamestGUI(modelindex).experiment(experimentindices(i)).MEXmodelfullpath = mexmodelfullpaths{i};
        end
    end
else
...
for k=1:length(experiments),
    ...
    for k2=1:length(experiments(k).measurements),
        ...
        timevector = unique([0 timevector(:)' timevectormeasurement(:)']);
    ...
    for k2=1:length(experiments(k).measurements),
        ...
        correcttimevectorindices = zeros(size(timevectormeasurement));
        for timeIndex = 1 : numel(timevectormeasurement)
            correcttimevectorindices(timeIndex) = find(modexpmeasinfostruct(k).timevector == timevectormeasurement(timeIndex));
        end
        timevectorindices = correcttimevectorindices;
        modexpmeasinfostruct(k).measurement(k2).timevectorindices = timevectorindices;
        timescaling = gettimescaling(timevectormeasurement);
