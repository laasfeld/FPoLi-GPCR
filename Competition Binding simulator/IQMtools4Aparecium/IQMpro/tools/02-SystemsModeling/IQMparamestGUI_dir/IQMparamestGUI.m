...
if get(handles.useExactEstimation, 'Value')
    p = struct(handles.UserData.project);
    estimationIndex = get(handles.listofestimations, 'Value');
    estimation = p.estimations{estimationIndex};
else
    if get(handles.globalLocalSystem, 'Value')
...
if get(handles.useExactEstimation, 'Value')    
    ...
    if get(handles.globalLocalSystem, 'Value')
        estimation = getEstimationStructure(handles);
    elseif get(handles.advancedGroupingSystem, 'Value')
        estimation = getEstimationStructureAdvancedGrouping(handles);    
    end
end
...
try
    ...
    global doPrecompile
    doPrecompile = false;
    if isequal(get(handles.openManualTuning, 'Value'), true)        
        doPrecompile = true;
    end
    if get(handles.globalLocalSystem, 'Value')
        output = IQMparameterestimation(handles.UserData.project,estimation,1);
    elseif get(handles.advancedGroupingSystem, 'Value')
        output = IQMparameterestimationAdvancedGrouping(handles.UserData.project,estimation,1);
    end
...
if isequal(get(handles.openManualTuning, 'Value'), true)
    ...
    try
        ...
        handles.UserData.project = IQMmanualtuning(handles.UserData.project,estimation);
        compiledExpModelsIQMparamestGUI = [];
    catch
         errordlg(lasterr);         
         compiledExpModelsIQMparamestGUI = [];
    end
...
handles.UserData.estdata = estdata;
try
   estdata.parameters{end+1} = 'Ki';
   estdata.Popt(:, size(estdata.Popt, 2)+1) = estdata.Popt(:, 2)./estdata.Popt(:,1); 
   estdata.parameters{end+1} = 'pKi';
   estdata.Popt(:, size(estdata.Popt, 2)+1) = log10(estdata.Popt(:, end))-9;
catch
...
dirname = uigetdir('\\172.17.6.224\Users\Tõnis\M1 receptor SB comaptible MIDAS files\IQMTools import files', 'Pick an IQMprojectSB folder');
if dirname == 0,
    return
end
% load project
...
elseif ~isempty(experimentindex),
    ...
    keepindices = [];
    for est=1:length(ps.estimations),
        for experimentindex = experimentindex
...
ps = struct(handles.UserData.project);
% get model file
[filename, pathname] = uigetfile( {'*.txt;*.txtbc;*.xml', 'Model Files (*.txt,*.txtbc,*.xml)'},'Pick a model file to import','\\OMERO\Users\Tõnis\M1 receptor SB comaptible MIDAS files\models');
if filename == 0,
    return
end
% load model
...
function duplicateModel_Callback(hObject, eventdata, handles)
m = get(handles.listofmodels,'Value');
...
% append the model to the project
IQMstructure = IQMstruct(ps.models{m});
IQMstructure.name = [IQMstructure.name, ' copy'];
ps.models{end+1} = IQMmodel(IQMstructure);
% update project
handles.UserData.project = IQMprojectSB(ps);
% update GUI
handles = UpdateProjectInformation(hObject, eventdata, handles);
...
% Update handles structure
if isequal(numel(indices), 1)
    ps = struct(handles.UserData.project);
    experiment = ps.experiments(indices).experiment;
    % convert to text
    expTextStructure = convertExpToTextIQM(experiment);
    exptext = setPartsToCompleteTextExpIQM(expTextStructure);
    set(handles.experimentPreview, 'String', exptext);
    %set(handles.experimentPreview, 'String', '');
end
...
estimation.experiments.measurementweight = {};
...
function estimation = getEstimationStructureAdvancedGrouping(handles)
estimation = [];
% check if model and experiments in project
project = handles.UserData.project;
ps = struct(project);
...
estimation.parameters.names = cell(0, 0);
estimation.parameters.lowbounds = [];
estimation.parameters.highbounds = [];
estimation.parameterslocal.names = cell(0, 0);
estimation.parameterslocal.lowbounds = [];
estimation.parameterslocal.highbounds = [];
% handles.advancedGroupingStruct.resultTable = resultTable;
% handles.advancedGroupingStruct.swappedParameters = swappedParameters;
% handles.advancedGroupingStruct.minMaxNominalTables = minMaxNominalTables;
paramLowArray = cell(numel(handles.advancedGroupingStruct.minMaxNominalTables), 1);
paramHighArray = cell(numel(handles.advancedGroupingStruct.minMaxNominalTables), 1);
paramGroupName = cell(numel(handles.advancedGroupingStruct.minMaxNominalTables), 1);
paramNominalValue = cell(numel(handles.advancedGroupingStruct.minMaxNominalTables), 1);
for i = 1 : numel(handles.advancedGroupingStruct.minMaxNominalTables)
    paramLowArray{i} = cell2mat(handles.advancedGroupingStruct.minMaxNominalTables{i}(:, 2));
    paramHighArray{i} = cell2mat(handles.advancedGroupingStruct.minMaxNominalTables{i}(:, 3));
    paramGroupName{i} = handles.advancedGroupingStruct.minMaxNominalTables{i}(:, 1);
    paramNominalValue{i} = cell2mat(handles.advancedGroupingStruct.minMaxNominalTables{i}(:, 4));
end
estimation.parametersadvanced.names = result.paramnames;
estimation.parametersadvanced.paramGroupNames = paramGroupName;
estimation.parametersadvanced.lowbounds = paramLowArray;
estimation.parametersadvanced.highbounds = paramHighArray;
estimation.parametersadvanced.nominalValue = paramNominalValue;
estimation.parametersadvanced.parameterGroupingTable = handles.advancedGroupingStruct.resultTable(estimation.experiments.indices, :);
estimation.initialconditions.names = result.icnames;
estimation.initialconditions.lowbounds = result.iclow;
estimation.initialconditions.highbounds = result.ichigh;
% options related
...
function sendToWorkspace_Callback(hObject, eventdata, handles)
assignin('base', 'tempVariable', eval(get(handles.variableName, 'String')));
finalName = get(handles.variableInWorkspace, 'String');
get(handles.variableInWorkspace, 'String');
evalin('base', [finalName, '= tempVariable']);
evalin('base', 'clear tempVariable');
return

function advancedGrouping_Callback(hObject, eventdata, handles)
% get the model
ps = struct(handles.UserData.project);
m = get(handles.listofmodels,'Value');
model = ps.models{m};
modelStruct = IQMstruct(model);
% find state names in the model
stateNames = cell(numel(modelStruct.states), 1);
ODE = cell(numel(modelStruct.reactions), 1);
for stateIndex = 1 : numel(modelStruct.states)
    stateNames{stateIndex} = modelStruct.states(stateIndex).name;
    ODE{stateIndex} = modelStruct.states(stateIndex).ODE;
end
parameterNames = cell(numel(modelStruct.parameters), 1);
for parameterIndex = 1 : numel(modelStruct.parameters)
   parameterNames{parameterIndex} = modelStruct.parameters(parameterIndex).name;
end
reactionNames = cell(numel(modelStruct.reactions), 1);
reactionFormulae = cell(numel(modelStruct.reactions), 1);

for reactionIndex = 1 : numel(modelStruct.reactions)
   reactionNames{reactionIndex} = modelStruct.reactions(reactionIndex).name;
   reactionFormulae{reactionIndex} = modelStruct.reactions(reactionIndex).formula;
end
% find state names in the experiments
statesInExperiments = cell(0, 0);
experimentNames = cell(0, 0);
statesInSpecificExperiment = cell(numel(ps.experiments), 1);
for experimentIndex = 1 : numel(ps.experiments)
    experimentStruct = IQMstruct(ps.experiments(experimentIndex).experiment);
    statesInSpecificExperiment{experimentIndex} = cell(numel(experimentStruct.paramicsettings), 1);
    for paramIndex = 1 : numel(experimentStruct.paramicsettings);
        statesInExperiments{end + 1} = experimentStruct.paramicsettings(paramIndex).name; 
        statesInSpecificExperiment{experimentIndex}{paramIndex } = experimentStruct.paramicsettings(paramIndex).name;
    end
    experimentNames{end + 1} = ps.experiments(experimentIndex).name;
end
statesInExperiments = unique(statesInExperiments);
paramInfo = getParamInfo(handles);
model = IQMgetmodel(handles.UserData.project, get(handles.listofmodels, 'Value'));
modelAsStruct = IQMstruct(model);
parameters = modelAsStruct.parameters;
parameterNominalValues = zeros(numel(parameters), 1);
for paramIndex = 1 : numel(parameters)
   parameterNominalValues(paramIndex) = parameters(paramIndex).value; 
end

[resultTable, swappedParameters, minMaxNominalTables] = AdvancedGroupingGUI(stateNames, statesInExperiments, parameterNames, reactionNames, reactionFormulae, ODE, experimentNames, paramInfo, parameterNominalValues, statesInSpecificExperiment);
handles.advancedGroupingStruct.resultTable = resultTable;
handles.advancedGroupingStruct.swappedParameters = swappedParameters;
handles.advancedGroupingStruct.minMaxNominalTables = minMaxNominalTables;
handles = swapStateNames(handles);
guidata(hObject, handles);
return

function handles = swapStateNames(handles)

projectAsStruct = IQMstruct(handles.UserData.project);
handles.experimentsBeforeSwap = cell(numel(projectAsStruct.experiments), 1);
% loop all the states of all the experiments and swap the states
for experimentIndex = 1 : numel(projectAsStruct.experiments)
    % process the initial conditions
    
    experimentAsStruct = IQMstruct(projectAsStruct.experiments(experimentIndex).experiment);
    handles.experimentsBeforeSwap{experimentIndex} = experimentAsStruct;
    tempVariable = struct2cell(experimentAsStruct.paramicsettings);
    stateNames = squeeze(tempVariable(1,1,:)); % extract the names of the variables
    finalStateNames = stateNames;
    for stateNameIndex = 1 : numel(stateNames)
        for stateIndex = 1 : size(handles.advancedGroupingStruct.swappedParameters, 1)
            if strcmp(handles.advancedGroupingStruct.swappedParameters{stateIndex, 1}, stateNames{stateNameIndex})
                finalStateNames{stateNameIndex} = handles.advancedGroupingStruct.swappedParameters{stateIndex, 2};
                break;
            end
        end
    end
    for stateIndex = 1 : numel(experimentAsStruct.paramicsettings)
        experimentAsStruct.paramicsettings(stateIndex).name = finalStateNames{stateIndex};
    end
    % process the state events
    for stateEventIndex = 1 : numel(experimentAsStruct.stateevents)
        variable = experimentAsStruct.stateevents(stateEventIndex).assignment.variable;
        formula = experimentAsStruct.stateevents(stateEventIndex).assignment.formula;
        newVariable = variable;
        newFormula = formula;
        for stateIndex = 1 : size(handles.advancedGroupingStruct.swappedParameters, 1)
            if strcmp(handles.advancedGroupingStruct.swappedParameters{stateIndex, 1}, variable)
                newVariable = handles.advancedGroupingStruct.swappedParameters{stateIndex, 2};
                newFormula = regexprep(formula, variable, newVariable);
                break;
            end
        end
        experimentAsStruct.stateevents(stateEventIndex).assignment.variable = newVariable;
        experimentAsStruct.stateevents(stateEventIndex).assignment.formula = newFormula;        
    end
    projectAsStruct.experiments(experimentIndex).experiment = IQMexperiment(experimentAsStruct);    
end
handles.UserData.project = IQMprojectSB(projectAsStruct);

return

function handles = resetStateNames(hObject, eventdata, handles)
    projectAsStruct = IQMstruct(handles.UserData.project);
    for experimentIndex = 1 : numel(projectAsStruct.experiments)
        projectAsStruct.experiments(experimentIndex).experiment = IQMexperiment(handles.experimentsBeforeSwap{experimentIndex});
    end
    handles.UserData.project = IQMprojectSB(projectAsStruct);
    guidata(hObject, handles);
return
...
function SaveFitanalysis(hObject, eventdata, handles)
    estdata = handles.UserData.estdata;
    if isempty(estdata),
    errordlg('You need to first run the fit analysis.');
    return
    end
    [file, path] = uiputfile('*.mat','Choose file location');
    fullpath = strcat(path, file);
    save(fullpath, 'estdata');
return
...
function LoadFitanalysis(hObject, eventdata, handles)
% clear the old project
%handles = clearProject(hObject, eventdata, handles);
% load new estdata and project
    [file, path] = uigetfile;
    fullpath = fullfile(path, file);
    estdata = load(fullpath);
    handles.UserData.estdata = estdata.estdata;
    % Update handles structure
    guidata(hObject, handles);
return