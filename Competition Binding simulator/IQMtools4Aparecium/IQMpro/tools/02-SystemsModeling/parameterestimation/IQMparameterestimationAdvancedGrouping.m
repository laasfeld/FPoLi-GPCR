function [output] = IQMparameterestimationAdvancedGrouping(project,estimation,varargin)
...
global PICmin PICmax parameterslocal integratoroptions parametersAdvancedGrouping
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[model,experiments,parametersAdvancedGrouping,initialconditions,optimization,costfunction,estimation,experimentmeasurementweights] = checkandprocessinputAdvancedGrouping(project,projectstruct,estimation);
...
[workestimation, parametersAdvancedGrouping] = addparameterindicesAdvancedGrouping(workestimation,estimation.parametersadvanced);
...
indexhi = find(parametersAdvancedGrouping.pliv(:)>=parametersAdvancedGrouping.plhigherbounds(:));
parametersAdvancedGrouping.pliv(indexhi) = parametersAdvancedGrouping.plhigherbounds(indexhi)*(1-tol);
indexlo = find(parametersAdvancedGrouping.pliv(:)<=parametersAdvancedGrouping.pllowerbounds(:));
parametersAdvancedGrouping.pliv(indexlo) = parametersAdvancedGrouping.pllowerbounds(indexlo)*(1+tol);
...
if optimizerboundsFlag == 1,
    X0 = [parametersAdvancedGrouping.pliv(:); initialconditions.ic0(:)];
else
...
if optimizerboundsFlag == 1,
    ...
    OPTIONS.lowbounds = [parametersAdvancedGrouping.pllowerbounds(:); initialconditions.iclowerbounds(:)];
    OPTIONS.highbounds = [parametersAdvancedGrouping.plhigherbounds(:); initialconditions.icupperbounds(:)];
    %%[OPTIONS.lowbounds(:), X0(:), OPTIONS.highbounds(:)]    
end
warning off
[Xopt,FVALopt] = feval(optimization.method,@costFunctionInterfaceAdvancedGrouping,X0,OPTIONS);
warning on
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if optimizerboundsFlag == 1,
    ParametersAdvancedOpt = Xopt(:);
    ICopt = Xopt(numel(parametersAdvancedGrouping.names)+1:end);
else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[paramreporttext] = boundsconstructreportAdvancedParameters(parametersAdvancedGrouping, ParametersAdvancedOpt);
...
% OUTPUT ARGUMENTS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
output = constructoutputAdvancedGrouping(projectstruct,estimation,parametersAdvancedGrouping,initialconditions,experiments,workestimation,ParametersAdvancedOpt,ICopt,FVALopt);
